...
| | |
|---|---|
| **Status** | Draft; **§3 log streaming + §4 acknowledgment modes shipped** (T7.1, FR-100): the commit log IS the stream ([`fluxum_server::replication`](../../crates/fluxum-server/src/replication.rs)) — raw STG-011 frames off the primary's durable segments, byte-identical replica logs (REP-010, e2e-pinned over the real wire), server-peer sessions (REP-005/REP-011), full sync via checkpoint-pack transfer + tail (REP-012), partial sync from offset (REP-013 — the divergence-truncation repair waits on T7.2's demote flow; a diverged replica is refused), replica apply with full index maintenance + local fan-out (REP-014/REP-043), heartbeats (REP-016), windowed flow control (REP-017), `async` + `semi_sync` with the quorum visibility barrier withholding ReducerResult/admin ack/TxUpdate (REP-020/021), `ack_timeout_ms` + `block`\|`degrade` quorum loss (REP-022), persisted epochs unified with the PITR lineage (REP-004), mechanical fencing on any channel + stale-batch rejection (REP-031 — demote/step-down is T7.2), and REP-081 metrics (`offset`/`lag_tx`/`connected`/`fenced_total`/`semi_sync_wait_us`/`degraded`). **§8 Backup + §9 PITR shipped** (T7.3, FR-103/FR-104): `fluxum backup create/verify/restore` over [`fluxum_core::backup`](../../crates/fluxum-core/src/backup/mod.rs) — hot create (REP-060/061), REP-062 archival wired into the boot-spawned checkpoint worker (`replication.archive.*` config, retention sweep, `fluxum_archive_segments_pending`), bit-flip-precise verify (REP-064), exact-head restore (REP-063), PITR to `--to-tx-id`/`--to-timestamp` with boundary report, chain-gap refusal, roll-forward guard, and the REP-072 `pitr.lineage` fencing-epoch marker the next boot adopts (`POST /checkpoint` serves `--fresh-checkpoint`). **§5 failover + §6 read replicas shipped** (T7.2, FR-101/FR-102): OQ-8 resolved as a minimal custom Raft-style election (votes only — the commit log already IS the replicated log) in [`fluxum_server::election`](../../crates/fluxum-server/src/election.rs) — majority votes (REP-030), a `(epoch, voted_for)` ballot persisted before it is answered (REP-004), the `(last_log_epoch, last_tx_id)` up-to-dateness rule and leader stickiness, promotion (REP-032) and automatic failover (a 3-member drill kills the primary, a replica wins and serves writes with zero committed-tx loss), demote-on-fence (REP-031: a stale primary rejoins as replica), write refusal on a replica (`CLUSTER_NOT_PRIMARY` redirect, REP-042), `ReplicaStale` read admission past `max_staleness_ms` (REP-041), the `/health` replication object (REP-080) and role/epoch/elections/full_syncs metrics (REP-081) + events (REP-082). Read + subscription fan-out already run on replicas (REP-043, from T7.1). The diverged-suffix truncation (REP-013) resyncs on restart (a live in-process store cannot be rebuilt in place). |
