//! The burst-mode fan-out driver (phase9 F-015) behind `fluxum-bench
//! fanout-burst` — in the library, like [`crate::soak`], so the smoke test
//! drives the same code path the operator command runs.
//!
//! `writers` identities fire one `send_chat` each, simultaneously, every
//! `writers / rate` seconds: each round lands a W-commit burst in every
//! subscriber's outbound queue while the aggregate rate stays at `rate` and
//! each identity stays under the demo module's 20/s admission ceiling.
//! Per-delivery e2e latency rides the message content (send instant, same
//! machine, same clock — the parity e2e's trick); the OBS-024 batch factor
//! is scraped from `/metrics` after the drain (`None` on a server that
...
pub struct FanoutConfig {
    ...
    pub writers: usize,
    /// Offered aggregate commit rate, commits/second.
    pub rate: u32,
    /// The measured window.
    pub duration: Duration,
}

/// The report `fanout-burst` writes (JSON is the artifact of record).
...
pub struct FanoutReport {
    ...
    pub sent: u64,
    /// Deliveries expected (`sent × subscribers`).
    pub expected: u64,
    /// Deliveries observed within the drain window.
    pub deliveries: u64,
    ...
    pub frames_per_write: Option<f64>,
    /// OBS-024 coalesced-frames raw counter, when present.
    pub writer_frames: Option<u64>,
    /// OBS-024 writes raw counter, when present.
    pub writer_writes: Option<u64>,
}

impl FanoutReport {
    /// The one-line human summary the command prints.
    #[must_use]
    pub fn summary_line(&self) -> String {
        format!(
            "fanout: {} sent, {}/{} delivered ({:.0}/s) | e2e p50 {} µs p95 {} µs p99 {} µs | frames/write {}",
            self.sent,
            self.deliveries,
            self.expected,
    ...
    /// Persist the JSON artifact under `out`, returning its path.
    pub fn persist(&self, out: &std::path::Path) -> Result<std::path::PathBuf, String> {
        std::fs::create_dir_all(out).map_err(|e| e.to_string())?;
        let path = out.join("fanout-report.json");
        let rendered = serde_json::to_string_pretty(self).map_err(|e| e.to_string())?;
        std::fs::write(&path, rendered).map_err(|e| e.to_string())?;
        Ok(path)
    }
}

/// Percentiles of the per-delivery e2e latency.
#[derive(Debug, serde::Serialize)]
...
/// Run one burst profile against an already-running server (the command
/// boots one; the smoke test spawns the release binary).
pub fn run_fanout_burst(
    cfg: &FanoutConfig,
    tcp_url: &str,
    http_port: u16,
) -> Result<FanoutReport, String> {
    let subscribers = cfg.subscribers.max(1);
    let writers = cfg.writers.max(1);
    let rate = cfg.rate.max(1);
    let duration = cfg.duration.max(Duration::from_secs(1));
    ...
    for i in 0..writers {
        writer_conns.push(
            Connection::connect(tcp_url, format!("fanout-writer-{i}").as_bytes(), [])
    ...
    let counters = scrape_frames_per_write(http_port);
    #[allow(clippy::cast_precision_loss)]
    Ok(FanoutReport {
        harness_version: 1,
        subscribers,
