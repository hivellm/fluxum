...
pub async fn dispatch(ctx: &Arc<ShardContext>, req: AdminRequest<'_>) -> AdminResponse {
    ...
    let response = match (method, split_path(path).as_slice()) {
        ...
        ("POST", ["rows"]) => row_edit(ctx, body).await,
...
/// (SHD-012).
async fn row_edit(ctx: &Arc<ShardContext>, body: &[u8]) -> AdminResponse {
    let (request_id, payload) = match parse_request(body) {
        Ok(pair) => pair,
        Err(e) => return AdminResponse::err(400, None, e),
    };
...
fn parse_decimal(s: &str) -> Result<fluxum_core::types::Decimal, String> {
