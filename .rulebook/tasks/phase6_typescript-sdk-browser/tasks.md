...
- **Unit 3 (next)** — 1.5 + 1.6, the refcounted cache and reconnect/reconciliation. The transport
  seam it builds on is deliberately frame-shaped: correlating an `id` back to a caller (RPC-002)
  is identical on both carriers, so it belongs above `Transport`, not inside each one.
