...
**Follow-up: delta compression** — promoted to its own task,
`phase9_delta-compression` (honor the dormant `tx_updates: "light"`
RPC-035 form, implement the negotiated per-session compression, spec
the column-level update deltas; per-client acked baselines rejected).
Sequenced after this task: the burst bench and MMO-shape workload
built here are the attribution rig for every layer there — batch ×
light × compression compose, and landing them together makes the wins
unattributable.
