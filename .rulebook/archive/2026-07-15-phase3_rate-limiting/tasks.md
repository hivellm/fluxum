...
- [x] 1.6 Gate G3 input: rate-limit suite green — full workspace suite (incl. rate-limit conformance) green locally with fmt/clippy clean and 91.86% line coverage; per maintainer directive (2026-07-15) GitHub Actions is suspended on quota exhaustion, so local validation IS the gate; re-run the 3-OS matrix via workflow_dispatch when Actions returns
