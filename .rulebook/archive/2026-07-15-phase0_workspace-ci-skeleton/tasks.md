...
- [x] 1.2 Align CI toolchain with the PRD section 9 constraint: workflows honor `rust-toolchain.toml` (nightly, edition 2024) instead of pinning stable; fmt + clippy `-D warnings` on all targets/features — landed in `27a70ef` (rust-lint/rust-test/simd-matrix install via bare `rustup toolchain install` from the file); all 4 workflows green on `17fb8d1` (Lint, Tests 3-OS, Codespell, SIMD Matrix x86-64+aarch64)
